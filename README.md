# Multimodal & Agentic AI Paper Review KO

Multimodal AI, Agentic AI 분야의 주요 논문을 한국어로 리뷰하고 정리하는 저장소입니다.

📖 **문서 사이트**: [multimodal-agentic-ai-paper-review-ko.readthedocs.io](https://multimodal-agentic-ai-paper-review-ko.readthedocs.io/ko/latest/)

---

## 카테고리

| 분류 | 설명 |
|------|------|
| **VLM** | Vision-Language Model |
| **LLM** | Large Language Model |
| **VLA** | Vision-Language-Action |
| **Agent AI** | LLM 기반 에이전트 |

## 리뷰 구성

각 논문 리뷰는 아래 항목을 중심으로 정리합니다.

- **핵심 아이디어** — 논문이 해결하려는 문제와 제안 방법의 직관
- **방법론** — 모델 구조, 학습 방식, 주요 설계 선택
- **실험 결과** — 주요 벤치마크 성능 및 ablation
- **한계점** — 논문이 인정하거나 드러난 제약
- **인사이트** — 후속 연구 연결고리 및 개인 코멘트

---

## 리뷰 로드맵

**범례**: ⬜ 예정 &nbsp;|&nbsp; 🔄 진행중 &nbsp;|&nbsp; ✅ 완료

### VLM (Vision-Language Model)

| 상태 | 논문 | 기관 | 발표 |
|:----:|------|------|------|
| ⬜ | CLIP | OpenAI | ICML 2021 |
| ⬜ | ALIGN | Google | ICML 2021 |
| ⬜ | Flamingo | DeepMind | NeurIPS 2022 |
| ⬜ | BLIP | Salesforce | ICML 2022 |
| ⬜ | BLIP-2 | Salesforce | ICML 2023 |
| ⬜ | LLaVA | - | NeurIPS 2023 |
| ⬜ | InstructBLIP | Salesforce | NeurIPS 2023 |
| ⬜ | GPT-4V | OpenAI | 2023 |
| ⬜ | LLaVA-1.5 | - | 2024 |
| ⬜ | InternVL2 | Shanghai AI Lab | 2024 |
| ⬜ | Qwen2-VL | Alibaba | 2024 |
| ⬜ | PaliGemma | Google | 2024 |
| ⬜ | Molmo | AI2 | 2024 |
| ⬜ | Cambrian-1 | NYU | 2024 |
| ⬜ | Janus | DeepSeek | 2024 |
| ⬜ | Qwen2.5-VL | Alibaba | 2025 |
| ⬜ | Gemini 2.0 | Google | 2025 |

### LLM (Large Language Model)

| 상태 | 논문 | 기관 | 발표 |
|:----:|------|------|------|
| ✅ | Transformer | Google | NeurIPS 2017 |
| ⬜ | GPT-3 | OpenAI | NeurIPS 2020 |
| ⬜ | T5 | Google | JMLR 2020 |
| ⬜ | InstructGPT | OpenAI | NeurIPS 2022 |
| ⬜ | Chain-of-Thought | Google | NeurIPS 2022 |
| ⬜ | LLaMA | Meta | 2023 |
| ⬜ | LLaMA 2 | Meta | 2023 |
| ⬜ | Mistral 7B | Mistral AI | 2023 |
| ⬜ | LLaMA 3 | Meta | 2024 |
| ⬜ | Qwen2.5 | Alibaba | 2024 |
| ⬜ | DeepSeek-V3 | DeepSeek | 2024 |
| ⬜ | Gemma 2 | Google | 2024 |
| ⬜ | Phi-3 | Microsoft | 2024 |
| ⬜ | o1 | OpenAI | 2024 |
| ⬜ | DeepSeek-R1 | DeepSeek | 2025 |
| ⬜ | Kimi k1.5 | Moonshot | 2025 |
| ⬜ | Llama 4 | Meta | 2025 |

### VLA (Vision-Language-Action)

| 상태 | 논문 | 기관 | 발표 |
|:----:|------|------|------|
| ⬜ | Gato | DeepMind | 2022 |
| ⬜ | SayCan | Google | CoRL 2022 |
| ⬜ | RT-1 | Google | 2022 |
| ⬜ | PaLM-E | Google | ICML 2023 |
| ⬜ | RT-2 | Google DeepMind | CoRL 2023 |
| ⬜ | Open X-Embodiment | 여러 기관 | 2023 |
| ⬜ | Octo | UC Berkeley | 2024 |
| ⬜ | SpatialVLM | Google | CVPR 2024 |
| ⬜ | OpenVLA | Stanford | 2024 |
| ⬜ | π0 | Physical Intelligence | 2024 |
| ⬜ | CogACT | 清华大学 | 2024 |
| ⬜ | RoboVLMs | - | 2024 |
| ⬜ | π0.5 | Physical Intelligence | 2025 |
| ⬜ | Helix | Figure AI | 2025 |

### Agent AI

| 상태 | 논문 | 기관 | 발표 |
|:----:|------|------|------|
| ⬜ | ReAct | Princeton / Google | ICLR 2023 |
| ⬜ | Toolformer | Meta | NeurIPS 2023 |
| ⬜ | Generative Agents | Stanford | UIST 2023 |
| ⬜ | Voyager | NVIDIA et al. | 2023 |
| ⬜ | AutoGen | Microsoft | 2023 |
| ⬜ | AgentBench | 清华大学 | ICLR 2024 |
| ⬜ | WebArena | CMU et al. | 2024 |
| ⬜ | OSWorld | 여러 기관 | 2024 |
| ⬜ | CogAgent | 清华大학 | CVPR 2024 |
| ⬜ | SWE-agent | Princeton | 2024 |
| ⬜ | OpenHands | - | 2024 |
| ⬜ | Claude Computer Use | Anthropic | 2024 |
| ⬜ | Browser Use | - | 2024 |
| ⬜ | Agentless | UIUC | 2024 |
| ⬜ | Manus | - | 2025 |

